...
def get_embedding(model, G, node2idx):
    emb = {}
    if not os.path.exists('../saved_dict/Struc2Vec/Struc2Vec.ckpt'):
        print('please train before!')
        return
    model.net.load_state_dict(torch.load('../saved_dict/Struc2Vec/Struc2Vec.ckpt'), False)
    model.net.eval()
    all_nodes = G.nodes()
